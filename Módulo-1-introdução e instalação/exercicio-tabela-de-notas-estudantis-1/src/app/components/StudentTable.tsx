...
export const StudentTable = ({ students }: Props) => {
    return (
        <table className="w-full border border-gray-600 rounded-md overflow-hidden">
            ...
            <tbody>
                {students.map(item => (
                    <tr className="text-grey-800 bg-grey-400 border-gray-600" key={item.id}>
                        ...
                        <td className="text-black p-3">{item.grade1.toFixed(1)}</td>
                        <td className="text-black p-3">{item.grade2.toFixed(1)}</td>
                        <td className="text-black p-3 font-bold">{item.active ? ((item.grade1 + item.grade2) / 2).toFixed(1) : '--'}</td>
                    </tr>
                ))}
            </tbody>
        </table>
